...
const inputCls = "w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-400 transition bg-white text-gray-900 placeholder-gray-400 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-100 dark:placeholder-zinc-500";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-zinc-100 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100">
            <div className="flex justify-between items-center mb-5">
              <h3 className="font-bold text-lg flex items-center gap-2 text-zinc-900 dark:text-zinc-50">
                <CalendarDays size={18} className="text-purple-500" /> Đặt lịch xem phòng
              </h3>
              <button onClick={() => setShowScheduleModal(false)} className="text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 transition">
                <X size={20} />
              </button>
            </div>
            {submitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-1">Quản lý sẽ liên hệ bạn sớm</p>
              </div>
            ) : (
              <div className="space-y-3">
                {[
                  ...
                  <div key={field.key}>
                    <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">{field.label}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{ label: "Số người", key: "people" }, { label: "Số xe", key: "vehicles" }].map((f) => (
                    <div key={f.key}>
                      <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">{f.label}</label>
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">Ghi chú</label>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-zinc-100 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowRegisterModal(false)} className="text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 transition">
                <X size={20} />
              </button>
            </div>
             <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900/30 rounded-xl p-3 mb-4 text-sm flex flex-col gap-1.5 text-purple-900 dark:text-purple-300">
              <div className="flex items-center gap-2">
                <Users size={14} className="text-purple-500" />
                <span><strong>{room.name}</strong> — {room.price.toLocaleString("vi-VN")}đ/người/tháng</span>
              </div>
              <div className="text-xs text-purple-700 dark:text-purple-400 mt-1 border-t border-purple-200/50 dark:border-purple-800/30 pt-1 font-medium">
                👉 Tiền đặt cọc phòng: <span className="font-bold text-sm text-purple-800 dark:text-purple-300">{room.price.toLocaleString("vi-VN")}đ</span> (sẽ hoàn lại khi dọn đi và kết thúc hợp đồng).
              </div>
            </div>
            {submitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-zinc-400 dark:text-zinc-505 mt-1">Admin sẽ xét duyệt và tạo hợp đồng</p>
              </div>
            ) : (
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">Số người *</label>
                  <select value={registerForm.people} onChange={(e) => setRegisterForm({ ...registerForm, people: e.target.value })} className={inputCls}>
                    {["1","2","3","4"].map(n => <option key={n}>{n}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">CCCD *</label>
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">Ngày chuyển vào *</label>
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block text-zinc-700 dark:text-zinc-300">Xe (nếu có)</label>
                ...
                {/* Điều khoản & Nội quy */}
                <div className="border border-purple-100 dark:border-purple-900/30 rounded-xl p-3 bg-purple-50/50 dark:bg-purple-950/10">
                  <div className="text-xs font-bold text-purple-950 dark:text-purple-300 mb-1.5 flex items-center gap-1.5">
                    📜 Điều khoản & Nội quy Ký túc xá
                  </div>
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-400 max-h-24 overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
                  ...
                  <label className="flex items-start gap-2 mt-3 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-xs text-zinc-700 dark:text-zinc-300 font-medium leading-tight">
