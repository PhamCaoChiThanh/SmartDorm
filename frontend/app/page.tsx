...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Load thêm phòng từ owner listings
      ...
      <section id="rooms" ref={roomsRef} className="mesh-bg py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((room) => (
              <div key={room.id} className="rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition group border border-gray-100">
                <div className="relative overflow-hidden h-52">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className={`text-xs px-3 py-1 rounded-full font-semibold ${room.available > 0 ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
                    <div>
                      <span className="text-blue-600 font-bold text-lg">{room.price.toLocaleString("vi-VN")}đ</span>
                      <span className="text-xs text-gray-400">/tháng</span>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => router.push(`/rooms/${room.id}`)}
                        className="text-sm px-3 py-2 rounded-xl font-medium transition"
                      ...
                      <button
                        ...
                        className="text-sm px-3 py-2 rounded-xl font-medium text-white transition"
          ...
          <div className="rounded-3xl overflow-hidden relative" style={{ background: "linear-gradient(135deg, #0F172A 0%, #1E1B4B 50%, #2D1B69 100%)" }}>
            ...
            <div className="relative z-10 p-12 lg:p-16 text-center">
              ...
              <div className="flex gap-4 justify-center flex-wrap">
                {/* buttons commented out intentionally */}
              </div>
            </div>
          </div>
        </div>
