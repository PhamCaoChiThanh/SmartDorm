...
export default function TenantInvoice() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white shadow-sm px-4 py-4 flex justify-between items-center">
        <h1 className="text-lg font-bold text-blue-600">🏠 SmartDorm</h1>
        <button
  ...
  >
    ...
    <div className="font-medium text-sm">Báo hỏng</div>
    <div className="text-xs text-gray-400">Gửi yêu cầu sửa chữa</div>
  </button>
  <button
    onClick={() => router.push("/tenant/parking")}
    className="bg-white rounded-xl shadow-sm p-4 text-left hover:shadow-md transition"
  >
    <div className="text-2xl mb-1">🚗</div>
    <div className="font-medium text-sm">Đăng ký xe</div>
    <div className="text-xs text-gray-400">Quản lý phương tiện</div>
  </button>
  <button
  ...
  >
    ...
    <div className="text-xs text-gray-400">Xem chi tiết hợp đồng</div>
  </button>
</div>
      </div>
    </div>
